...
Each run caps sources, response bytes (at most 1 MB), and timeout (at most 30 seconds). Each source also requires one to five unique operator-declared `lanes` (at most 40 characters each). Adapters copy these labels unchanged; they do not infer classification or quality. All adapters emit the same minimized `Evidence` fields: stable content-derived ID/digest, source and item URL, adapter, lanes, retrieval time, visibility, title, and summary. Authorization configuration is absent in this public slice.
